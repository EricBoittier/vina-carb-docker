...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
			   ...
			   int seed, int verbosity, bool score_only, bool local_only, tee& log, const terms& t, const flv& weights, std::vector< std::vector<size_t*> > glycoinfo,fl chi_coeff,fl chi_cutoff) {
		char* filename;
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
				 ...
				 const flv& weights,
				 int cpu, int seed, int verbosity, sz num_modes, fl energy_range, tee& log, std::vector< std::vector<size_t*> > glycoinfo, fl chi_coeff, fl chi_cutoff) {
	...
	if(randomize_only) {
		...
		if(no_cache) {
			do_search(m, ref, wt, prec, nc, prec_widened, nc_widened, nc,
					  ...
					  seed, verbosity, score_only, local_only, log, t, weights, glycoinfo,chi_coeff,chi_cutoff);
		}
		else {
...
int main(int argc, char* argv[]) {
	...
	const std::string version_string = "Vina-Carb 1.0 (August 30, 2015)";
	const std::string error_message = "\n\n\
Please contact the author, Anita K. Nivedha <nivedha@uga.edu>, so\n\
...
store.clear();
		...
		main_procedure(m, ref, 
					...
					gd, exhaustiveness,
					weights,
					cpu, seed, verbosity, max_modes_sz, energy_range, log, glycoinfo,chi_coeff,chi_cutoff);
	}
	catch(file_error& e) {
